"""Define TexErrorContainer class.

Classes
-------
TexErrorContainer
    Configures TexErrorContainer frame.
"""
...
class TexErrorContainer(ttk.Frame):
    ...
    highlighted_frame : ttk.Frame()
        reference to frame that has highlighted text lines
    ...
    error_close()
        Removes highlight from lines in editor and destroys TexErrorContainer frame
    """

    def __init__(self, container, controller, **kwargs):
        """Initialises TexErrorContainer.
        ...
        **kwargs
        ...
        self.log_file_name = ""
        # Create variable to store reference to frame with highlighted lines
    ...
    def retrieve_error_message(self, log_file_name, log_file_path, tex_file, tex_file_name):
        ...
        tex_file_name : str
            tex file name
        """
        # Makes scrolled text writable and clears contents
        self.error_frame.config(state="normal")
        self.error_frame.delete("1.0", "end")

        # Calls retrieve_error_message with passed log file name and path, and tex file name and path
        ...
        # Removes highlighted lines if they exist and then calls self._highlight_error_lines to highlight new lines
        if self.highlighted_frame:
            self.highlighted_frame.tex_editor.tag_delete("highlightline")
        self._highlight_error_lines(line_numbers)
    ...
    def _format_error_message(self, errors):
        ...
        """
        # Call format_error_message from files/error_functions.py
        line_numbers = err.format_error_message(self, errors)
        return line_numbers

    def _highlight_error_lines(self, line_numbers):
        """Highlights lines in scrolled text where errors occurred
        ...
        line_numbers : list
            list of line numbers where errors occurred
        """
        self.highlighted_frame = self.controller.visible_tex_frame
        for line in line_numbers:
            self.highlighted_frame.tex_editor.tag_add(
                "highlightline",
                f"{line}.0", f"{int(line) + 1}.0"
            )

            self.highlighted_frame.tex_editor.tag_configure(
                "highlightline",
                background="#ffcccb"
            )
    ...
    def error_close(self):
        """Removes highlight from lines in editor and destroys TexErrorContainer frame"""
        self.controller.tex_paned_window.forget(self)
        self.highlighted_frame.tex_editor.tag_delete("highlightline")
    ...
    def _save_log_as(self):
        ...
        box
        """
        # Call save_log_as from files/error_functions.py
